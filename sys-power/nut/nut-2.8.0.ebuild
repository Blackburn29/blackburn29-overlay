...
IUSE="cgi ipmi snmp +usb selinux split-usr ssl tcpd xml zeroconf man"
...
src_configure() {
	...
	if use man; then
		myconf="${myconf} --with-doc=man"
	else
		myconf="${myconf} --with-doc=no"
	fi
